package jp.co.internous.team2506.model.mapper;
...
public interface TblCartMapper {
	 ...
	 * @return 件数
	 */
	@Select(value="SELECT product_count FROM tbl_cart WHERE user_id = #{userId}")
	 ...
	 * @return 件数
	 */
	@Select(value="SELECT product_count FROM tbl_cart WHERE user_id = #{userId} AND product_id = #{productId}")
